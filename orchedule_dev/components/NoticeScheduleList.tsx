    ...
    <section className="px-4 py-2">
      ...
      <div className="bg-white rounded-xl shadow p-2">
        {mockNotices.map((notice) => (
          <div key={notice.id} className="flex justify-between items-center p-1 text-sm">
            <div className="flex items-center gap-2">
                {notice.pinned ? (
                    <Image src="/icons/pin-filled.svg" alt="pinned" width={14} height={14} />
                ) : (
                    <Image src="/icons/pin.svg" alt="pinned" width={14} height={14} />
                )}
              <span>{notice.title}</span>
              {notice.isNew && <span className="text-xs text-red-500 ml-1">N</span>}
            </div>
  ...
  return (
    <section className="px-4 py-2">
      ...
      {mockSchedules.map((schedule) => (
        <div key={schedule.id} className="flex mb-3 rounded-xl overflow-hidden shadow bg-white">
          <div className="w-16 bg-[#a08e8e] text-white text-center flex flex-col justify-center items-center py-2">
            <span className="text-xs font-bold">{schedule.displayDate.split(' ')[0]}</span>
            <span className="text-sm font-bold">{schedule.displayDate.split(' ')[1]}</span>
          </div>
